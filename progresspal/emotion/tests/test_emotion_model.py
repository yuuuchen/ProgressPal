# emotion/tests/test_emotion_model.py
import unittest
import numpy as np
from unittest.mock import patch
from emotion.emotion_model import predict_emotion, InputShapeError


class TestEmotionModel(unittest.TestCase):

    def setUp(self):
        # 正確 shape 的輸入 (1,224,224,1)
    ...
    def test_invalid_input_shape(self, mock_model):
        """
        如果輸入影像 shape 不是 (1,224,224,1)，應該拋出 InputShapeError
        """
        mock_model.predict.return_value = np.zeros((1, 6))

        wrong_input = np.zeros((224, 224, 1), dtype=np.float32)
